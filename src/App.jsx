...
export default function App () {
  ...
  // FIXED: Proper scheduler that continues looping + reads live pattern/bpm/swing changes
  const scheduler = () => {
    ...
    while (nextNoteTimeRef.current < ctx.currentTime + lookahead && isPlayingRef.current) {
      ...
      // Apply swing to odd steps using current swing value
      if (currentSwing > 0 && (stepRef.current % 2 === 1)) {
        scheduleTime += (currentSwing / 100) * (stepDur / 3)
      }
      
      // Get current pattern from ref and schedule hits
      const currentPattern = patternRef.current
      TRACKS.forEach((t, row) => {
        if (currentPattern[row][stepRef.current]) {
          scheduleHit(ctx, master, t.type, scheduleTime, volumes[row], sharedNoise)
        }
      })
